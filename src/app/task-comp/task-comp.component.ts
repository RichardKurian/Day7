...
public sameCeoResultsTwo:any={};

public sameCeotwo() {
  this.sameCeoResults = Object.keys(this.secondObj).reduce((acc:any, tenant) => {
      const ten = this.secondObj[tenant];
      if (!acc[ten.CEO]) {
          acc[ten.CEO] = [];
      }
      acc[ten.CEO].push({ tenant, name: ten.name });
      return acc;
  }, {});
  
  this.sameCeoResults = 
  Object.entries(this.sameCeoResults)
  ...
  console.log("same ceo results:", this.sameCeoResults);
}
